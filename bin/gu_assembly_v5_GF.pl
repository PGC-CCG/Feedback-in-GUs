#################################################################################
###### 05-09-15                                                            ######
###### EDITED BY Georgette Femerling on 05-08-19
###### GU Assembly                                                         ######
...
######   v5 > Does not take into consideration the effect of the           ######
######      > interaction                                                  ######
###### Notes:                                                              ######
###### - Requires Perlcyc & Pathway tools installed                        ######
###### - Pathey Tools should be running in -api -lisp mode                       ######
#################################################################################
...
sub effector {
  ...
  while(<TRN>){
    if($_=~/^$TF\t([^\t]+)\[[^\t]/i){ ###EDICION DE GEORGETTE PARA VER SI ESTO JALA SIN EFECTO
    #if($_=~/^$TF\t([^\t]+)\[[^\t]+\t([^\t]+)/i){
      $tu=$1;
      $func="?";
      #$func=$2;
