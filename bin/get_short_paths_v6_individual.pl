...
###### v4.1/18-12-19/> Edited by Georgette Femerling                       ######
###### v4.1/18-12-19/> Made it to run individual TF, added arguments       ######
###### [0] - TF gu_assembly_vX.pl output directory                            ######
###### [1] - TF to calculate short paths                                   ######
###### [2] - Output file path                                              ######
###### [3] - Path minimum length (use "1" for connectivity,                ######
######       "0" for components search to include disconnected rxns)       ######
#################################################################################
...
#Get args
$dir_path=$ARGV[0]; #./datasets/GUs_secondary_reactions/TF
$TF=$ARGV[1]; #TF
$out_file=$ARGV[2]; #all_short_paths_TF or short_paths_conn_TF
$limit=$ARGV[3];
chomp($limit);
...
#foreach $file (@GU_files){
 # if(!($file=~/^\./)){
    ...
    $products=$dir_path . "reactants_products.txt";
    $objects=$dir_path . "objects.txt";
    $reactions=$dir_path . "reactions.txt";
